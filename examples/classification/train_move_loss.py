...
def main(gpu, cfg, profile=False):
    ...
    saliency_model = None
    if hasattr(cfg, "saliency_model"):
        if not cfg.saliency_model.get('criterion_args', False):
            cfg.saliency_model.criterion_args = cfg.criterion_args
        saliency_model = build_model_from_cfg(cfg.saliency_model).to(cfg.rank)
        if cfg.saliency_model.get('in_channels', None) is None:
            cfg.saliency_model.in_channels = cfg.saliency_model.encoder_args.in_channels
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        # set_temperature
        temperature = get_temperature(epoch, cfg.epochs)
        
        model.encoder.resampler.set_temperature(temperature)
        
        if not (hasattr(cfg, "skip_train") and cfg.skip_train):
            train_loss, move_loss, train_macc, train_oa, _, _ = \
                train_one_epoch(model, train_loader,
                                optimizer, scheduler, epoch, cfg, saliency_model=saliency_model)
        
        # set 0 when inference
        # model.encoder.resampler.set_temperature(0)
        ...
        if epoch % cfg.val_freq == 0:
            ...
            if hasattr(cfg, "skip_train") and cfg.skip_train:
                return
...
def train_one_epoch(model, train_loader, optimizer, scheduler, epoch, cfg, saliency_model=None):
    ...
    for idx, data in pbar:
        ...
        if cfg.get("saliency", False):
            if saliency_model is None:
                saliency_model = model
            points.requires_grad = True
            data['pos'] = points[:, :, :3].contiguous()
            data['x'] = points[:, :, :cfg.saliency_model.in_channels].transpose(1, 2).contiguous()

            if hasattr(cfg, "saliency_model"):
                _, task_loss = saliency_model.get_logits_loss(data, target) if not hasattr(saliency_model, 'module') else saliency_model.module.get_logits_loss(data, target)
            else:
                _, _, task_loss, _ = saliency_model.get_logits_loss(data, target) if not hasattr(saliency_model, 'module') else saliency_model.module.get_logits_loss(data, target)
            
            task_loss.backward()

            pos = points
            pos_grad = pos.grad
            from ..saliency_utils import cal_saliency_score
            saliency_score = cal_saliency_score(pos[:, :, :3], pos_grad[:, :, :3]).detach()

            if cfg.get("saliency_rev", False):
                sorted_indices = torch.argsort(saliency_score, dim=1, descending=True)
            else:
                sorted_indices = torch.argsort(saliency_score, dim=1)
            optimizer.zero_grad()
            pos.grad.zero_()
            pos.requires_grad = False
            points = torch.gather(points, dim=1, index=sorted_indices.expand(-1, -1, points.shape[-1]))
            points.requires_grad = False
        
        data['pos'] = points[:, :, :3].contiguous()
        data['x'] = points[:, :, :cfg.model.in_channels].transpose(1, 2).contiguous()
        logits, loss, task_loss, move_loss = model.get_logits_loss(data, target) if not hasattr(model, 'module') else model.module.get_logits_loss(data, target)
        ...
        if idx % cfg.print_freq == 0:
            pbar.set_description(f"Train Epoch [{epoch}/{cfg.epochs}] "
                                 f"Loss {loss_meter.val:.3f} move_loss {move_loss_meter.avg:.3f} Acc {cm.overall_accuray:.3f} T {model.encoder.resampler.temperature}")
