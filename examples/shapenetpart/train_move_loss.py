...
def main(gpu, cfg):
    ...
    for epoch in range(cfg.start_epoch, cfg.epochs + 1):
        ...
        model_module.encoder.resampler.set_temperature(temperature)

        train_loss, move_loss, task_loss = \
            train_one_epoch(model, train_loader, criterion,
                            optimizer, scheduler, epoch, cfg)
        
        # set 0 when inference
        # model_module.encoder.resampler.set_temperature(0)
        ...
        if writer is not None:
            ...
            writer.add_scalar('task_loss', task_loss, epoch)
...
def train_one_epoch(model, train_loader, criterion, optimizer, scheduler, epoch, cfg):
    ...
    for idx, data in pbar:
        ...
        move_loss_meter.update(move_loss.item(), n=batch_size)
        if idx % cfg.print_freq:
            pbar.set_description(f"Train Epoch [{epoch}/{cfg.epochs}] "
                                 f"Loss {loss_meter.avg:.5f} Move_Loss: {move_loss_meter.avg:.5f} Task_Loss: {(loss_meter.avg - move_loss_meter.avg):.5f}"
                                 )
    train_loss = loss_meter.avg
    
    return train_loss, move_loss_meter.avg, train_loss - move_loss_meter.avg
